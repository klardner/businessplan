...
Our cover page can be accessed below. 
```{r download button cover page , echo = FALSE, warning=FALSE}
library(downloadthis)
download_file(
  path = "C:\\Users\\kiera\\Documents\\R\\RProjects\\businessplan\\coverpage.docx",
  output_name = "Group 24C- Business Plan Cover Page",
  button_label = "Group 24C - Cover Page",
  button_type = "primary",
  has_icon = TRUE,
  icon = "fa fa-save",
...
For the best viewing experience, please open this file in your preferred web browser. 
...
While there is no requirement that partnership arrangements must be in writing, the Partners have agreed that certain provisions of the [Partnership Act, 1890](http://www.irishstatutebook.ie/eli/1890/act/39/enacted/en/print.html) (the "**1890 Act**") are not appropriate for the modern management structure that is envisaged by ELHQ Solicitors.
...
::: {.clearfix}
<img src="kieran.jpg" alt="Pineapple" class="img5" width="170" height="160"/>
...
Kieran spent a year working in the Property department at McCann Fitzgerald prior to commencing his traineeship.
</li>
...
Furthermore, each partner brings experience in key areas the firm intends on being a market leader in, resulting in a nice balance of knowledge across practice areas.
...
Our strengths and weaknesses are set out in [Appendix 2](#appendix2)
...
node [shape = box ,
      ...
      label = '']
node [fillcolor = red]
1[ label = 'Edward Hynes - Managing Partner & Head of Banking']
node [fillcolor = green]
2[ label = 'Conor Quane - Head of Corporate & Finance ']
3[ label = 'Samuel Elliott - Head of Litigation & Employment']
4[ label = 'Kieran Lardner - Head of Property & Construction']
...
Please feel free to visit [it](https://klardner.shinyapps.io/clientappdemo/) We believe this will improve productivity across the firm and ultimately deliver a more cost-efficient and effective service to our clients.
...
ELHQ plans to employ one legal secretary and bookkeeper initially.
These positions will be advertised on the [law society website](https://lawsociety.ie) and through recruitment websites such as irishjobs.ie.
...
Kieran Lardner holds a certificate in Technology Law from the Law Society and CIPP/E and CIPM certifications.


#### Human Resources Manager

Kieran Lardner will act as the Human Resources Manager.
He will be responsible for the recruitment, training and development of all employees and will ensure compliance with the Employment Equality Acts 1988 - 2015.
...
Firm policies have been drafted and are set out in [Appendix 4](#appendix4).
...
ELHQ recognises, in particular in light of COVID-19 that this is a dynamic obligation and will ensure compliance with the [Health, Safety and Welfare at Work Act 2005](https://www.irishstatutebook.ie/eli/2005/act/10/enacted/en/print) as well as any guidance issued by the HSE and Department of Health.
ELHQ will ensure that 1 Percy Place maintains an up-to-date Health and Safety Risk Assessment.
...
A breakdown of our costs by category in our first year can be seen in the below chart. 


```{r, echo = FALSE, warning= FALSE, message=FALSE, dpi=100}
flow <- data.frame(year = c(1,1,1,1,1,2,2,2),
                      category = c("Expenditure","Expenditure", "Receipts","Receipts","Receipts", "Expenditure", "Receipts","Net Cash Flow"),
                      group = c("Ongoing expenditure", "Set-up costs", "Fees","Capital Contributions", "Term Loan", "Ongoing Expenditure","Fees", "Net"),
                      value = c(13986,415002,298340,120000,40000,512414,571668,59254))

# saveRDS(flow,paste0("cashflowplotly",".rds"))
...
flow1 <- flow %>% filter(year == 1)
# ggplotly(
#   ggplot(flow1, aes(x=category, y=value, text = value, fill = group)) + 
#     geom_bar(stat='identity')  +
#     scale_y_continuous(labels=dollar_format(suffix="",prefix="€"))+
#     labs(y = "",title =  "ELHQ - Year 1 Expenditure vs Receipts",
#          subtitle = "Hover to see breakdown")+
#     theme_fivethirtyeight()+
#     theme(legend.position = 'none'),
#   tooltip = "text"
# )

plot <- ggplot(flow1, aes(x=category, y=value, text =  paste("Category:", category, "\nSub-Category:",group, "\nValue: €",value ), fill = group)) + 
    geom_bar(stat='identity')  +
    scale_y_continuous(labels=dollar_format(suffix="",prefix="€"))+
    labs(y = "",title =  "ELHQ - Year 1 Expenditure vs Receipts")+
    theme_fivethirtyeight()+
...
# 
# P1 <- ggplotly(plot, tooltip = "text") %>%
# config(displayModeBar = FALSE)
# %>%
# layout(title = list(text = paste0('<b>ELHQ - Year 1 Expenditure vs Receipts</b>',
#                                   '<br>',
#                                   '<sup>',
#                                    '<i>Hover groups to see breakdown</i>','</sup>')))
# 
# P1
...
In year 2, there are no set-up costs and the only source of income is fee income. The below chart shows our net cash flow in Year 2. 


```{r, echo = FALSE, warning= FALSE, message=FALSE, out.width="100%"}
flow2 <- flow %>% filter(year == 2)

flow2$category = factor(flow2$category, levels = c("Expenditure","Receipts","Net Cash Flow"),
labels = c("Expenditure","Receipts","Net Cash Flow"))

library(forcats)
library(ggtext)

flow3 <- flow2 %>% mutate(category = fct_reorder(category, desc(value)))

# plot2 <- ggplot(flow3, aes(x=category, y=value)) + 
#     geom_bar(stat='identity')  +
#     scale_y_continuous(labels=dollar_format(suffix="",prefix="€"))+
#     labs(y = "",title =  "ELHQ - Year 2 Expenditure vs Receipts",
#          subtitle = "Hover to see breakdown")+
#     theme_fivethirtyeight()+
#     theme(legend.position = 'none')

plot2 <- ggplot(flow3, aes(x=category, y=value, text =  paste("Category:", category, "\nValue: €",value ))) + 
    geom_bar(stat='identity', fill = c("#1b9e77","#d95f02","#7570b3"))  +
    scale_y_continuous(labels=dollar_format(suffix="",prefix="€"))+
    labs(y = "",title =  "<b><span style = 'color:#000000;'>ELHQ Year 2 - </span><span style = 'color:#1b9e77;'>Receipts</span>, <span style = 'color:#d95f02;'> Expenditure </span> & <span style = 'color:#7570b3;'>Net Cash Flow </span></b>")+
    theme_fivethirtyeight()+
    theme(legend.position = 'none', plot.title = element_markdown())


# P2 <- ggplotly(plot2, tooltip = "text") %>%
#   layout(title = list(text = paste0("<b><span style = 'color:#000000;'>ELHQ Year 2 - </span><span style = 'color:#1b9e77;'>Receipts</span>, <span style = 'color:#d95f02;'> Expenditure </span> & <span style = 'color:#7570b3;'>Net Cash Flow </span></b>",                                    '<br>',
#                                     '<sup>',
#                                      '<i>Hover to see value</i>','</sup>')))
...
```{r, eval = FALSE, echo = FALSE}
 ...
 objectname = data 
objectname %>% 
ggplot(aes(x = xvar, y = round(yvar,2), color = facetvar)) + 
geom_line(size = 1.3) + 
geom_point()  + 
geom_label(aes(x = xvar+5, y = yvar, label = as.character(yvar), fill = facetvar), color = 'black', hjust = 0.5, family = "Consolas", alpha = .5)+ 
transition_reveal(xvar) + 
facet_wrap(~facetvar, nrow = 2,scales = "free_y")  +
scale_color_manual(values = c("color1", "color2", "color3", "color4"))  +
scale_fill_manual(values = c("color1", "color2", "color3","color4")) +
scale_x_continuous(limits = c(limits), breaks = seq(breaks)) +
...
plot.title = element_text(face = 'italic',hjust = .5,size = 12), 
plot.subtitle = element_markdown(hjust = .5,size = 10), 
legend.text = element_text(size = 12),
strip.text = element_text(size = 7),
strip.background.x = element_rect(fill = "#E5E1D8"),
panel.spacing.x = unit(1, 'lines'),
plot.caption = element_text(face = 'bold', hjust = c(0,1)))+ 
labs(subtitle = "<b><span style = 'color:color1;'>Set-up costs </span> , <span style = 'color:color2;'>category2 </span> , <span style = 'color:color3;'>category3 </span> & <span style = 'color:color4;'>category4 </span></b>", 
title = paste0("title \n (As of ", format(Sys.Date(), format="%B %d, %Y"), ")"),
x = "xlab", 
y = "ylab", 
caption = c("Credit: Kieran Lardner @lardner23", "Source: fbref.com"))
...
Kieran Lardner has designed the logo.

<img src="logo2.png" alt="Firm Logo"/>
...
Kieran Lardner and our Legal Technology team have designed the [website](https://elhq.netlify.app/)
The Firm's website has been designed in such a way that it is easy to navigate as well as being informative.

Information has been provided about each of the Partners, and contact details for each individual Partner will be provided.The website will has an 'insights' section where we will  regular articles on topical legal issues. The website also has a 'case studies' section where we can showcase the ways we've helped our clients. 
...
Eddie Hynes, Managing Partner, has a contact who has agreed to set up the website and podcast hosting platforms for €1,500 at a once off cost.
...
```{r download button Partnership Agreement , echo = FALSE, warning=FALSE}
library(downloadthis)
download_file(
  path = "C:\\Users\\kiera\\Documents\\R\\RProjects\\businessplan\\partnershipagreement.pdf",
  output_name = "ELHQ - Draft Partnership Agreement",
  button_label = "Download Draft Partnership Agreement as PDF",
  button_type = "danger",
  has_icon = TRUE,
  icon = "fa fa-save",
  self_contained = FALSE
...
```
The Draft Partnership Agreement provided by the law society accesible [**here**](https://lawschool.lawsociety.ie/pluginfile.php/110696/mod_resource/content/1/partnership-agreement.pdf)forms the basis of the agreement between each of the partners, subject to any amendments noted below:
...
<!-- -->
```
a.  "**Commencement Date**" - shall mean the 1 July 2021;
b.  " **Financial Year**" - shall mean the period from 1 July to 30 June in the next year (inclusive) or any such period as the Partners may determine by Resolution from time to time; and
c.  " **Premises**" - shall mean the location from which ELHQ Solicitors ordinarily conducts business and such other additional or alternative place or places as the Partners shall from time to time and for the time being by Resolution determine.
...
ELHQ Employment contracts for both fixed-term and permanent employees have been prepared. They can be downloaded below. 

```{r download button employment perm , echo = FALSE, warning=FALSE}
library(downloadthis)
download_file(
  path = "C:\\Users\\kiera\\Documents\\R\\RProjects\\businessplan\\contractperm.doc",
  output_name = "ELHQ Template Employment Contract - Permanent",
  button_label = "ELHQ Employment Contract - Permanent",
  button_type = "primary",
  has_icon = TRUE,
  icon = "fa fa-save",
  self_contained = FALSE
...
```{r download button employment fixed , echo = FALSE, warning=FALSE}
library(downloadthis)
download_file(
  path = "C:\\Users\\kiera\\Documents\\R\\RProjects\\businessplan\\contractfixed.docx",
  output_name = "ELHQ Template Employment Contract - Fixed",
  button_label = "ELHQ Employment Contract - Fixed-term",
  button_type = "primary",
  has_icon = TRUE,
  icon = "fa fa-save",
  self_contained = FALSE
...
A number of policies have been prepared for the firm. They can be accessed below. 

```{r download button dignity policy, echo = FALSE, warning=FALSE}
library(downloadthis)
download_file(
  path = "C:\\Users\\kiera\\Documents\\R\\RProjects\\businessplan\\dignitypolicy.pdf",
  output_name = "ELHQ - Dignity Policy",
  button_label = "Dignity Policy",
  button_type = "danger",
  has_icon = TRUE,
  icon = "fa fa-save",
  self_contained = FALSE
...
```{r download button grievance prodcedure, echo = FALSE, warning=FALSE}
library(downloadthis)
download_file(
  path = "C:\\Users\\kiera\\Documents\\R\\RProjects\\businessplan\\grievanceprocedure.pdf",
  output_name = "ELHQ - Grievance Procedure",
  button_label = "Grievance Procedure",
  button_type = "danger",
  has_icon = TRUE,
  icon = "fa fa-save",
  self_contained = FALSE
...
```{r download button disciplinary procedure, echo = FALSE, warning=FALSE}
library(downloadthis)
download_file(
  path = "C:\\Users\\kiera\\Documents\\R\\RProjects\\businessplan\\disciplinaryprocedure.pdf",
  output_name = "ELHQ - Disciplinary Procedure",
  button_label = "Disciplinary Procedure",
  button_type = "danger",
  has_icon = TRUE,
  icon = "fa fa-save",
  self_contained = FALSE
...
```{r download button maternity leave, echo = FALSE, warning=FALSE}
library(downloadthis)
download_file(
  path = "C:\\Users\\kiera\\Documents\\R\\RProjects\\businessplan\\maternityleave.pdf",
  output_name = "ELHQ - Maternity Leave Policy",
  button_label = "Maternity Leave Policy",
  button_type = "danger",
  has_icon = TRUE,
  icon = "fa fa-save",
  self_contained = FALSE
...
```{r download button parental leave, echo = FALSE, warning=FALSE}
library(downloadthis)
download_file(
  path = "C:\\Users\\kiera\\Documents\\R\\RProjects\\businessplan\\parentalleave.pdf",
  output_name = "ELHQ - Parental Leave Policy",
  button_label = "Parental Leave Policy",
  button_type = "danger",
  has_icon = TRUE,
  icon = "fa fa-save",
  self_contained = FALSE
...
```{r download button health and safety, echo = FALSE, warning=FALSE}
library(downloadthis)
download_file(
  path = "C:\\Users\\kiera\\Documents\\R\\RProjects\\businessplan\\healthandsafety.pdf",
  output_name = "ELHQ - Health and Safety",
  button_label = "Health & Safety",
  button_type = "danger",
  has_icon = TRUE,
  icon = "fa fa-save",
  self_contained = FALSE
